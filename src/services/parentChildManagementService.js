...
export const updateChild = async (parentId, childId, updateData) => {
  ...
  if (!child) {
    throw new ApiError(
      'Không tìm thấy tài khoản con hoặc bạn không có quyền sửa',
      400
    );
  }
...
export const getTimeLimit = async (childId, parentId, timeRange) => {
  ...
  const dailyStats = await db.UsageLog.findAll({
    ...
    attributes: [
      'session_date',
      [
        db.sequelize.fn('SUM', db.sequelize.col('active_seconds')),
        'total_seconds',
      ],
    ],
    group: ['session_date'],
  ...
  const totalMinutes = dailyStats.reduce(
    (sum, day) => sum + Math.floor(day.total_seconds / 60),
    0
  );
