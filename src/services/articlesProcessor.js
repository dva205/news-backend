...
export const listPendingArticles = async () => {
  const { data, error } = await supabase.storage.from(PENDING_BUCKET).list();
  if (error)
    throw new ApiError(`Lỗi khi list file từ bucket: ${error.message}`, 500);
  return data;
};
...
export const downloadArticle = async (fileName) => {
  ...
  if (error)
    throw new ApiError(`Lỗi khi tải file từ bucket: ${error.message}`, 500);
  return data;
};
