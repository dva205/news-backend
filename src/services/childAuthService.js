...
export const validateInviteCode = async (code) => {
  ...
  if (!parent || !child) {
    throw new ApiError(
      'Thông tin người mời hoặc người được mời không tồn tại',
      404
    );
  }
...
export const signInChild = async (username, password) => {
  ...
  if (!existChild.password_hashed) {
    throw new ApiError(
      'Tài khoản chưa được kích hoạt. Vui lòng kích hoạt tài khoản bằng mật khẩu mới.',
      403
    );
  }
