import {
  ...
  setStrict,
  updateChild,
} from '../services/parentChildManagementService.js';
...
export const getAllChild = async (req, res) => {
  try {
    ...
    return sendSuccess(
      res,
      data,
      `Bạn có ${data.children.length} tài khoản con`,
      200
    );
  } catch (error) {
...
export const setChildStrict = async (req, res) => {
  try {
    const childId = req.params.id;
    const parentId = req.user.id;
    const { timeLimit, blockedKeyword, blockedCategory, blockedFeature } =
      req.body;
    ...
    return sendSuccess(
      res,
      data,
      'Cập nhật các giới hạn cho con thành công',
      200
    );
  } catch (error) {
