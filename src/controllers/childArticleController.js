...
} from '../services/childArticleService.js';
import { checkBadWord, recommendArticle } from '../utils/aiService.js';
...
export const getRecommendArticle = async (req, res) => {
  try {
    const childId = req.user.id;
    const articleId = req.params.id;
    ...
    const data = await fetchRecommendArticles(
      articleId,
      recommendUrls,
      childId
    );
    ...
    console.error('Lỗi khi gọi recommend child article:', error);
    return sendError(res, error);
  }
};
