...
export const validateSession = async (req, res) => {
    try {
        ...
        const token = req.cookies?.refreshChildToken || req.cookies?.refreshParentToken;

        if (!token) {
            return sendSuccess(res, { hasSession: false, role: null }, "Không có phiên làm việc", 200);
        }
        ...
        if (!session) {
            ...
            return sendSuccess(res, { hasSession: false, role: null }, "Phiên làm việc không tồn tại trong hệ thống", 200);
        }

        // Có cookie = có session
        return sendSuccess(res, { hasSession: true, role: session.user.role }, "Phiên làm việc hợp lệ", 200);
